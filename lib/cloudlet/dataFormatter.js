   ...
   if (msg['content-type'] === "application/json-ld") {
      ...
      for (var i in context) {
         if (context.hasOwnProperty(i)) {
            var entry = context[i];
            var ldEntry = {'@id' : entry['@description'], '@type' : entry['@data_type'] };
...
var objectHelper = function(db_object, msg) {
   ...
   //var type = db_object.value['@type'];

   db_object.value.type = undefined;
   ...
   if (db_object.cas) {
      resp_obj["_revision"] = "" + db_object.cas;
   }

   if (undefined === db_object.value['@id']) {
      resp_obj = { '@id' : db_object.value[1]  };
   }
